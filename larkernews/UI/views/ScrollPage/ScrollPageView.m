...
-(UITableViewCell *)cellForRowInTableView:(UITableView *)aTableView IndexPath:(NSIndexPath *)aIndexPath FromView:(CustomTableView *)aView{
    ...
    LKNewsViewCell *vCell = [aTableView dequeueReusableCellWithIdentifier:vCellIdentify];
    if (vCell == nil) {
        vCell = [[[NSBundle mainBundle] loadNibNamed:@"LKNewsViewCell" owner:self options:nil] lastObject];
    }
    ...
    vCell.titleLabel.text = @"test shuju 我的测试数据标题";
    return vCell;
}
...
-(float)heightForRowAthIndexPath:(UITableView *)aTableView IndexPath:(NSIndexPath *)aIndexPath FromView:(CustomTableView *)aView{
    LKNewsViewCell *vCell = [[[NSBundle mainBundle] loadNibNamed:@"LKNewsViewCell" owner:self options:nil] lastObject];
